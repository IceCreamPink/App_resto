<?php 
if($id != ""){
    $sel = $dbo->select('menu where id='.$id);
    foreach($sel as $row){
...
if(isset($_POST['simpan'])){
    ...
    if($foto == ""){
        $up = $dbo->update("menu", "nama_menu='$nama_menu', deskripsi='$deskripsi', harga='$harga', idkategori='$idkategori'", "idmenu=$idmenu");
    }else{
          $nama_foto = date("YmdHis").$_FILES['foto']['name'];
        $nama_tmp = $_FILES['foto']['tmp_name'];
        $folder = '../img/';
        move_uploaded_file($nama_tmp, $folder.$nama_foto);
        $up = $dbo->update("menu", "nama_menu='$nama_menu', deskripsi='$deskripsi', harga='$harga', idkategori='$idkategori', foto='$nama_foto'", "idmenu=$idmenu");
    }
    if($up){
        ?>
<script>
...
?>
<div class="judul">
    <a href="?hal=menu" class="btn-add"><i class="fa fa-list"> Lihat Data </i></a>
    ...
    <form action="?hal=edit_menu" method="post" enctype="multipart/form-data">
        <table>
            ...
            <tr>
                ...
                <td>
                    <select name="kategori" id="" required>
                        ...
                        ?>
                        <option <?=$baris['idkategori']==$idkategori?'selected':''?> value="<?=$baris['idkategori']?>">
                            <?=$baris['kategori']  ?>
                        </option>
                        <?php 
                        }
            ...
            <tr>
                ...
                <td>:</td>
                <td>
                    <input type="text" name="deskripsi" placeholder="Deskripsi" value="<?=$desk?>" required>
                </td>
            </tr>
            <tr>
                <td>Harga</td>
